    ...
    public function load(
        ...
        foreach ($grns as $grn) {
            ...
            $grns2 = \App\Models\Grn::query()
                ->where('parent', "grn:inventory:namespace:{$namespaceName}")
                ->where('category', "inventoryModel")
                ->get();
            foreach ($grns2 as $grn2) {
                $inventoryModelName = $grn2->key;

                $grnKeys = GrnKeyLoader::load(
                    $this->createClient(),
                    new \App\Models\Grn([
                        'grn' => "{$grn['grn']}:user:$userId:inventoryModel:{$inventoryModelName}",
                        'parent' => $grn2['grn'],
                        'key' => $userId,
                    ]),
                    GrnKeyLoader::buildQuery(
