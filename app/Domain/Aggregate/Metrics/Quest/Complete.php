...
class Complete extends AbstractMetrics
{
    ...
    public function load(): LoadResult {
        ...
        " . implode(
                ...
                COUNTIF(CONCAT(
                    IFNULL(JSON_EXTRACT_SCALAR(request, '$.isComplete'), ''),
                    IFNULL(JSON_EXTRACT_SCALAR(JSON_EXTRACT_SCALAR(JSON_EXTRACT_SCALAR(JSON_EXTRACT_SCALAR(request, '$.stampSheet'), '$.body'), '$.args'), '$.isComplete'), ''),
                    IFNULL(JSON_EXTRACT_SCALAR(JSON_EXTRACT_SCALAR(JSON_EXTRACT_SCALAR(JSON_EXTRACT_SCALAR(request, '$.stampTask'), '$.body'), '$.args'), '$.isComplete'), '')
                ) = 'true') AS value,
            FROM
                `{$this->table()}`
            WHERE
        ...
        foreach ($items as $item) {
            ...
            $metrics[] = Metrics::query()->updateOrCreate(
                ["metricsId" => "$service:complete:count:$keys:{$item["date"]}"],
                [
                    "key" => "$service:complete:count:$keys",
                    "value" => $item["value"],
