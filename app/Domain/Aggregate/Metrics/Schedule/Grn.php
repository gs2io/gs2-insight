...
namespace App\Domain\Aggregate\Metrics\Schedule;
...
class Grn extends AbstractMetrics
{
    ...
    public function load(): LoadResult {
        ...
        foreach ($namespaces->grns as $namespace) {
            ...
            $totalBytesProcessed += $eventModels->totalBytesProcessed;

            $triggerModels = GrnLoader::load(
                $this->createClient(),
                $namespace,
                GrnLoader::buildQuery(
                    $service,
            ...
            $totalBytesProcessed += $triggerModels->totalBytesProcessed;
        }
